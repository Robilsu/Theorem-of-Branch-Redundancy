...
namespace aux
  ...
  | exp p1 p2 q := ( power (power p2 exp) q ) / ( power (power p1 exp) q )
...
namespace llot
  ...
  | mp mt (leaf LVL LBL) pth loop lvl lbl u_node := begin
    simp [loop_tree_level_label_option],
    from false.elim
  end
  | mp mt (leaf LVL LBL) pth loop lvl lbl lr_node := begin
    simp [loop_tree_level_label_option],
    from false.elim
  end
